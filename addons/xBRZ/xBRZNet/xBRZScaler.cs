    ...
    */
	...
	public class xBRZScaler
	{
		...
		*/
		...
		// scaleSize = 2 to 5
		public Image ScaleImage( Image image, int scaleSize, ScalerCfg config = null )
		{
			...
			GD.Print( "imageData.Length: " + imageData.Length );

			var rgbValues = new int[imageData.Length / 4];
			...
			ScaleImage( scaleSize, rgbValues, scaledRbgValues, image.GetWidth(), image.GetHeight(), config, 0, int.MaxValue );

			var finalData = new byte[scaledRbgValues.Length * 4];

			for ( var i = 0; i < scaledRbgValues.Length; i++ )
			{
				var color = scaledRbgValues[i];
			...
			var fixedFormatImage = image.Duplicate() as Image;
			fixedFormatImage.Convert( Image.Format.Rgba8 );

			// var imageData = image.GetData();
			var rgbValues = new int[fixedFormatImage.GetWidth() * fixedFormatImage.GetHeight()];
			/* for ( var i = 0; i < imageData.Length; i++ )
			{
				rgbValues[i] = (int)imageData[i];
			} *

			for ( var y = 0; y < fixedFormatImage.GetHeight(); y++ )
			{
				for ( var x = 0; x < fixedFormatImage.GetWidth(); x++ )
				{
					var color = fixedFormatImage.GetPixel( x, y );
					rgbValues[y * fixedFormatImage.GetWidth() + x] = color.
				}
			}
			...
			ScaleImage( scaleSize, rgbValues, scaledRbgValues, fixedFormatImage.GetWidth(), fixedFormatImage.GetHeight(), config, 0, int.MaxValue );

			var finalData = new byte[scaledRbgValues.Length];

			for ( var i = 0; i < scaledRbgValues.Length; i++ )
			{
				finalData[i] = (byte)scaledRbgValues[i];
			}

			var newImage = Image.CreateFromData( fixedFormatImage.GetWidth() * scaleFactor, fixedFormatImage.GetHeight() * scaleFactor, false, Image.Format.Rgba8, finalData );

			return newImage; */

			/* var rgbValues = Array.ConvertAll( fixedFormatImage.GetData(), c => (int)c );
			...
			var finalData = Array.ConvertAll( scaledRbgValues, c => (byte)c );
