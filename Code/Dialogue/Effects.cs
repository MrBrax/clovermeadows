...
namespace vcrossing.Code.Dialogue
{
	...
	public static class Effects
	{
		...
		public static async Task Typewriter( RichTextLabel text, float lettersPerSecond,
			Action<string> onCharacterTyped, TaskInterruptToken stopToken = null )
		{
			...
			TaskInterruptToken stopToken = null )
		{
			...
			while ( GodotObject.IsInstanceValid( text ) && text.VisibleRatio < 1 )
			{
				...
				while ( accumulator >= secondsPerLetter )
				{
					...
					var currentLetter = text.Text.Substring( Math.Max( text.VisibleCharacters - 1, 0 ), 1 );

					text.VisibleCharacters += 1;
					onCharacterTyped?.Invoke( currentLetter );
					accumulator -= secondsPerLetter;

					if ( text.VisibleCharacters >= characterCount )
					{
						continue;
					}

					// var nextChar = text.Text.Substring( text.VisibleCharacters, 1 );
					if ( currentLetter.Equals( "." ) || currentLetter.Equals( "," ) || currentLetter.Equals( "!" ) || currentLetter.Equals( "?" ) || currentLetter.Equals( ":" ) || currentLetter.Equals( ";" ) )
					{
						// accumulator += secondsPerLetter * 2;
						GD.Print( "Punctuation" );
						await Effects.InterruptableWait( 0.1f, stopToken );
						// await DefaultActions.Wait( 0.5f );
					}
